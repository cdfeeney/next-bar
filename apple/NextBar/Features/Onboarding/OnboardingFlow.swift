    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            .navigationDestination(for: OnboardingStep.self) { step in
                ...
                case .username:
                    UsernameView(onContinue: { path.append(.locationPrimer) })
                case .locationPrimer:
                    LocationPrimerView(onContinue: { path.append(.quiz) })
                case .quiz:
                    QuizView(onFinished: { profile in
                        Task {
                ...
                case .findFriends:
                    FindFriendsView(onDone: { appState.isSignedIn = true })
                }
            }
        }
    }
