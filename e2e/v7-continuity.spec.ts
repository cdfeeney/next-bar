...
test('V7 ratings, tied scores, lists, and night history survive navigation and reload', async ({
  page,
}) => {
  ...
  await expect(page.getByRole('button', { name: /^V7 favorites 2 bars$/ })).toBeVisible();
  await page.reload();
  await expect(page.getByRole('button', { name: /^V7 favorites 2 bars$/ })).toBeVisible();
