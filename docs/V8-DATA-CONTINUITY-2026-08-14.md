...
| `next-bar:started-night-out:v1` | session | V8-3b fix round 1: the id of a Night Out that was CREATED but never opened | **Local-only, never synced** — one tab's in-flight intent (a night_outs uuid), written by StartNightOutButton immediately after create_night_out succeeds and cleared the moment the plan is actually opened. It exists because component state died on unmount, re-arming Start and letting the next tap create a SECOND plan for the same night. sessionStorage on purpose: the plan is durable server-side, so this must not outlive the session or leak across tabs. Holds no account data. Not a V7 key. |
