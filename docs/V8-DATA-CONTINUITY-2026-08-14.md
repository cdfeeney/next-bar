...
| `next-bar:owned-night-out:v1` | local | V9-03 addition: the Night Out this account CREATED tonight, **keyed by account** | **Local-only, never synced.** Value is a MAP, `{ [userId]: { planId, nightKey } }`, the same shape as `started-night-out` and holding the same account identifiers — account data, and dropped by `destroyAccountDataOnDeletion` alongside the parked record (per-account entry when the id is known, the whole key otherwise). Written by StartNightOutButton right after `create_night_out` succeeds, beside the parked record; read by `YourPlanTonight` (Social → Plans) so the creator can find their plan again after leaving, reloading or reopening the app on this device, because `get_my_night_outs` excludes owned plans (0059:296). Deliberately a SECOND record with the opposite lifetime: the parked one is a create-recovery guard the plan page spends on open; this one lives for the whole night. Entries for other nights are pruned on every write; the card forgets an entry whose plan is gone or cancelled. Not a V7 key. |
...
| `next-bar:night-out-rsvp-queued-name:` | local, **one entry per share token** | G-01 addition: the guest NAME belonging to an offline-queued RSVP | **Local-only, never synced.** Going and Maybe carry the name the guest typed (0080); an answer queued offline must replay with it or the server refuses it, so the name is held beside the queued choice and cleared with it. Kept in its own key rather than inside the queued value so an older queue (a bare choice) still replays. Carries no identity of its own — the row it will become is keyed by the RSVP key above. Not a V7 key. |
