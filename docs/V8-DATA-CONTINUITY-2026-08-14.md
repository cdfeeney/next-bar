...
| `next-bar:account:deletion-uncertain:v1` | local | WP8 addition: the user id of an account whose deletion attempt ended with NO usable answer | **Local-only, never synced** — a device-side fact about what this browser knows, holding a server user ID. `/api/account/delete` destroys the auth user and only then writes its reply, so a response lost in between leaves a deleted account and a browser that cannot tell. From that moment Settings may never again print "nothing was removed" about that account: a later `unauthorized` is exactly what a DELETED user's token produces. Written by the danger zone on any `unknown` outcome; the VALUE is the user id it concerns, so a latch that outlives its session cannot make the next owner of this device cautious about an account nothing tried to delete. It is registered in `accountCache` `ALL_KEYS`, so a confirmed deletion and a sign-out seal both clear it — after a successful sign-in the account demonstrably exists and the uncertainty is over. Deliberately NOT per-user key names: the interpolation gate below forbids computed keys so entries stay visible to this registry. Not a V7 key. |
