...
| `next-bar:pending-invite:v1` | session | V8-3 addition: Night Out invite token riding the sign-in handoff | **Local-only, never synced** — one tab's in-flight intent (a share_token uuid), stored by the /night-out page before /auth and consumed exactly once by PendingInviteRedirect after sign-in. sessionStorage on purpose: must not leak across tabs or outlive the browser session. Holds no account data. Not a V7 key. |
| `next-bar:native-push-installation:v1` | local | V8-4 addition: this INSTALL's push identity (a uuid), the key `native_device_tokens` rotates a device token against | **Local-only, never synced** — it names the device, never the account, and holds no account data. Deliberately survives sign-out: the same phone signing back in must map to the same installation row rather than mint a second one and consume another slot against the per-user device cap. Regenerated on reinstall, which is correct — a reinstall genuinely is a new installation, and the save RPC transfers the reused APNs token off the stale row. Not a V7 key. |
| `next-bar:native-push-prompted:v1` | local | V8-4 addition: latch recording that iOS push permission has been requested once on this install | **Local-only, never synced** — device-side UI bookkeeping. Exists to enforce the PRD rule that permission is asked only after a user sends or accepts an invitation, and then at most once, never on first launch. Survives sign-out with the installation id above; the Settings "Enable on this device" button is the deliberate re-ask path. Not a V7 key. |
