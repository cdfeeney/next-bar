...
| `next-bar:account:deletion-uncertain:v1` | local | WP8 addition: the user id of an account whose deletion attempt ended with NO usable answer | **Local-only, never synced** — a device-side fact about what this browser knows, holding a server user ID. `/api/account/delete` destroys the auth user and only then writes its reply, so a response lost in between leaves a deleted account and a browser that cannot tell. From that moment Settings may never again print "nothing was removed" about that account: a later `unauthorized` is exactly what a DELETED user's token produces. Written by the danger zone on any `unknown` outcome; the VALUE is the user id it concerns, so a latch that outlives its session cannot make the next owner of this device cautious about an account nothing tried to delete. Read at the moment of each attempt, never cached into component state on mount — a snapshot cannot see an attempt another tab made afterwards. Cleared two ways, and BOTH are needed: `ALL_KEYS` membership covers the confirmed-deletion and foreign-sign-in wipes, and an explicit removal in `clearResidualAccountCache` covers the sign-out paths, which clear by named list and do not iterate `ALL_KEYS`. **Dropped only where the session is demonstrably gone** — `useAuth` calls that function directly when `getSession()` returns nothing or a `SIGNED_OUT` event arrives. The sign-out SEAL keeps it (`keepDeletionLatch`): the seal runs after `supabase.auth.signOut()` whatever that returned, and a failed sign-out resolves with `{ error }` while keeping the session, so sealing proves nothing about whether the session ended. Deliberately NOT per-user key names: the interpolation gate below forbids computed keys so entries stay visible to this registry. Not a V7 key. |
...
| `next-bar:night-out-rsvp:` | local, **one entry per share token** | WP7 addition: the capability identifying ONE token-scoped recipient's RSVP, per plan | **Local-only, never synced.** A uuid the recipient's own device mints, because `night_out_anon_rsvps` has no account to key on and `night_out_members` needs a profile id this person does not have (D-C-23). Holding the share token lets you RSVP; only this key reads back or changes the answer stored under it — so a forwarded link never hands anyone else your answer, and this is why the key is per plan rather than per device. Cleared storage or a private window simply means the next RSVP is a NEW recipient: nobody is locked out, and the server's per-plan cap is what bounds the growth that allows. Not account data and never uploaded — the server already holds the row it names. **Not covered by any wipe set on purpose:** it belongs to a signed-out visitor who has no account cache to wipe. Not a V7 key. |
| `next-bar:night-out-rsvp-queued:` | local, **one entry per share token** | WP7 addition: an RSVP taken while offline, held until it can be sent | **Local-only, never synced.** V8-R-INV-003's failure clause requires an offline answer to be queued and labelled as not yet sent; this is that queue, one held choice (`going` \| `maybe` \| `declined`) per plan. Cleared the moment the write lands — and also when the server REFUSES it, because holding a declined answer forever would keep promising a delivery that cannot happen. Carries no identity of its own: the row it will become is keyed by the RSVP key above. Not a V7 key. |
...
| `next-bar:stories-seen:v1` | local | V8-1f: ids of story items already watched on THIS DEVICE — the unseen RING's only source | **Local-only, never synced**, and as of migration 0065 the ONLY story key left. It is per-device READ STATE, not content: the story itself is a `public.stories` row with its bytes in the private `story-media` bucket, so nothing about a story is on this device to leak. Wiped by `guardAgainstForeignCache` on a FOREIGN sign-in (it is in `FOREIGN_ONLY_KEYS`, not `ALL_KEYS`), so it survives the device owner's own sign-out but never tells a second account which stories "you" had seen. Capped ring buffer. Not a V7 key. |
| `next-bar:stories:v1` | **RETIRED 2026-08-22** | was: your story items as photo data URLs | Retired by migration 0065. Stories are server-backed; signed-in stories are never browser data URLs. Removed from `FOREIGN_ONLY_KEYS` deliberately, not silently dropped. |
| `next-bar:story-replies:v1` | **RETIRED 2026-08-22** | was: replies you typed on a story or Feed memory | Retired with the Reply control itself. Nothing ever delivered these, so the button promised a message the product never sent; both return together when there is real delivery plus a surface where the author can read it (V9). |
| `next-bar:stories-untagged:v1` | **RETIRED 2026-08-22** | was: ids of stories you took your own tag off | Retired by migration 0065. Tag withdrawal is now a real backend relationship (`public.story_tags.removed_at` via `remove_my_story_tag`), so consent is enforced for everyone rather than suppressed on one device's read side. |
