...
describe('bounded route search', () => {
  ...
  it('stops at 20 elements, fills farther options honestly, preserves primary routes if secondary fails', async () => {
    ...
    expect(result.routes[1].walking?.seconds).toBe(1320);
    expect(result.routes.every(r => r.driving === null)).toBe(true);
    expect(result.incomplete).toBe(true);
    expect(result.limited).toBe(true);
    expect(fetcher.mock.calls.reduce((n, c) => n + JSON.parse(c[1].body).destinations.length, 0)).toBe(20);
  });
  ...
  it('uses drive routes for cab without excluding nearby candidates', async () => {
    const fetcher = vi.fn().mockResolvedValueOnce(matrix([50, 90, 40, 80, 70]))
      .mockResolvedValueOnce(matrix([100, 200, 300, 400, 500]));
    vi.stubGlobal('fetch', fetcher);
    const result = await searchRoutes(origin, bars, 'driving', false, 'key', signal());
    expect(fetcher.mock.calls[0][0]).toContain('driving-car');
    expect(result.routes.map(r => r.id)).toEqual(bars.slice(0, 5).map(b => b.id));
    expect(result.routes).toHaveLength(5);
  });
