    ...
    <main className="min-h-screen pb-28">
      ...
      <div className="max-w-md mx-auto px-6 mt-[18px] space-y-8">
        ...
        {tab === 'feed' ? (
          <Panel id="feed">
            {railFor('feed')}
            {/* FeedSection mounts whenever stories are READY. It is the only reader of
                persistent Feed posts, so gating its mount on the 24-hour story count
                (the old branch here) hid every post once the last story expired —
                and, from S-11, hid the post an author had just been told "See it"
                about. READY-AND-EMPTY is still its own state (feed-empty), said
                inside FeedSection once its own read has settled. */}
            {stories.status === 'ready' ? (
