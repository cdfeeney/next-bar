 ...
 * unfinished, and a parent must never record 'cleared' over one. It judges the
 * word against `isTerminalStatus`, the same vocabulary `classify` uses on the
 * recorded cell, so the outcome domain and the record domain cannot disagree
 * about which words end a cell.
 */
const isTerminalOutcome = isTerminalStatus;
...
async function processCell(cell, ctx) {
  ...
  // ONE classification, read by every branch below. The engine used to spell
  // out its own versions of these tests — the floor-plus-evidence rule, the
  // acknowledgement rule, the evidence rule — and each copy was free to lag the
  // invariant's. `Verdict` is the shared answer; there is nothing left here to
  // disagree with.
  const verdict = classify(known);
  ...
  // `atFloor` carries the evidence demand: a floor status is only ever written
  // after a capped response, so a real floor cell always has the successful
  // attempt that proves it. Trusting the bare word here meant a fabricated
  // floor DONE was skipped forever — never searched, never convergent, and
  // (before the waiver was reordered) acknowledgeable.
  if (verdict.atFloor) {
  ...
  if (known) {
    ...
    // `verdict.hasEvidence` is what keeps this from manufacturing it. A SUBDIVIDE
    ...
    if (known.capped && !known.children?.length && !verdict.blocked) {
  ...
  // `settledForResume`, not `completing`: a completing status that carries a
  ...
  if (known && verdict.settledForResume) {
