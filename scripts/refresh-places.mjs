...
/**
 * The ONLY place the sidecar is written. Every path funnels through here so the
 * invariants in lib/sidecar.mjs cannot be bypassed by a caller that forgets.
 */
function writeSidecar(patches, knownIds) {
  assertSidecarWritable(patches, knownIds);
  const entries = Object.keys(patches)
    .sort()
    .map((id) => `  '${id}': ${JSON.stringify(patches[id])},`);
  const out = [
...
(async () => {
  ...
  const patches = {};
  const flags = [];
  /** Bars rejected this run — their existing sidecar entry must be removed, not kept. */
  ...
  // targeted run can't wipe every other bar's data. mergeOnlyPatches also DROPS
  // anything rejected this run: `{ ...existing, ...patches }` used to carry the
  // stale entry of exactly the bar the location check had just rejected, so the
  // guard perpetuated the patch it was meant to reject.
  const finalPatches = ONLY ? mergeOnlyPatches(existing, patches, rejectedIds) : patches;
  writeSidecar(finalPatches, new Set(parseBars().map((b) => b.id)));
...
async function runPhotosMulti() {
  ...
  writeSidecar(existing, new Set(parseBars().map((b) => b.id)));
