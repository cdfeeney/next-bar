...
-- gets its own table and bucket rather than borrowing 0038's draft. EXIF/GPS
-- stripping is a server-side re-encode obligation on the upload path, recorded
-- there; a bucket policy cannot enforce it.
--
...
comment on function public.is_mutual_friend(uuid, uuid) is
  ...
  'follow is not a friend. Used by the stories RLS policies.';
...
-- Author sees its own, including expired ones (so "your story" can show its own
-- history to its owner within the app's own rules) — but NOT deleted ones.
...
comment on function public.publish_story is
  ...
  'verifies the media keys belong to the caller, and refuses a custom audience '
  'containing anyone who is not an accepted mutual friend.';
