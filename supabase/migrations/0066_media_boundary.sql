...
-- MOVED HERE BY THE 0066 REORDER, 2026-08-28. The block below is BYTE-IDENTICAL to what
-- previously sat under section 9; only its position changed.
--
-- WHY IT HAD TO MOVE: `media_path_unreported_live_expiry` below is LANGUAGE SQL, and
-- PostgreSQL validates a SQL function body at CREATE time. It selects from
-- public.content_reports, which section 9 did not create until ~1150 lines later, so
-- applying this migration to an empty database failed outright:
--     ERROR: relation "public.content_reports" does not exist
-- A plpgsql body would not have been validated and the fault would have shipped silently.
-- Text review cannot see this; it only appears when the migration is executed.
